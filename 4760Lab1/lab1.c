...
short reco_buff[32];
short freq_table[] = {0,262,294,330,349,392,440,494,523,0};
short replay;
...
void __ISR(_TIMER_2_VECTOR, ipl2) Timer2Handler(void)
{
  ...
  // output envelope for debugging
  DAC_data_B = (int) current_amplitude ;
  }

  if (curr_mode == FREQ_MODE){
    // frequency mode, without envelope control
    DAC_data_A = (int)(max_amplitude*sine_table[DDS_phase>>24]) + 2048;
  }
  
  if(Fout==0 || curr_mode == RECO_MODE) DAC_data_A = 0; 
...
static PT_THREAD (protothread_timer(struct pt *pt))
{
  ...
  while(1) {
    ...
    if (curr_mode == PLAY_MODE && replay){
      ...
      Fout = (note>11)? freq_table[note-12]*2 : freq_table[note];
...
static PT_THREAD (protothread_key(struct pt *pt))
{
  ...
  mPortYEnablePullUp(BIT_4 | BIT_5 | BIT_6 | BIT_7); //Enable pull-up resistors

  mPortZSetPinsIn(BIT_0 | BIT_1);    //Set port as input
  mPortZEnablePullUp(BIT_0 | BIT_1 | BIT_2 | BIT_3 | BIT_4 | BIT_5 | BIT_6 | BIT_7); //Enable pull-up resistors
